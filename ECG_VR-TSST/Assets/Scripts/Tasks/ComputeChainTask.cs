...
public class ComputeChainTask : AbstractTask
{

    [SerializeField]
    private TextSequencer[] _sequences;
    ...
    public override bool Next(Round round, bool secondRoundActive, bool secondRoundAvailable)
    {
        switch (_state)
        {
            ...
            case State.TaskAction:
                ...
                _state = State.ComputeChains;
                MainButtonSwitcher.Instance.Next(true);
                OptionalButtonSwitcher.Instance.ReenableButtons();
                return true;
            case State.ComputeChains:
                if (firstChain)
                {
                    firstChain = false;
                    WriteLogEntry();
                }
                if (_sequences[_sequenceState].Next()) return true;
                else if (_sequenceState == _sequences.Length - 1)
                {
                    _state = State.End;
                    return true;
                }
            ...
            case State.ThanksAction:
                ...
                return false;
            default:
                Debug.LogError("[ComputeChainTask] Undefined State");
                return false;
        }
    }
    ...
    void WriteLogEntry()
    {
        ...
        string textToWrite = $"Current task: \"ComputeChainTask\"; Current state of Task: \"{_state.ToString()}\" at {System.DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss")}\r\n";
